...
void DrawBonobono(HWND hwnd, HDC hdc, int blink)
{
    ...
    if (blink)
    {
        ...
        DeleteObject(hBrush);
        HBRUSH hEllipseBrush = CreateSolidBrush(RGB(127, 200, 255)); // 하늘색
        HBRUSH ddllipseBrush = CreateSolidBrush(RGB(255, 150, 255)); // 핑크색
        HBRUSH xxllipseBrush = CreateSolidBrush(RGB(255, 255, 255)); // 흰색
        SelectObject(hdc, hEllipseBrush); // 하늘색
        Ellipse(hdc, 250, 120, 550, 425); // 얼굴
        SelectObject(hdc, ddllipseBrush); // 핑크색
        Ellipse(hdc, 377, 244, 424, 322); // 입
        SelectObject(hdc, xxllipseBrush); // 흰색
        Ellipse(hdc, 353, 245, 400, 292); // 입 (왼쪽)
        Ellipse(hdc, 400, 245, 447, 292); // 입 (오른쪽)
        SelectObject(hdc, bBrush); // 검은색
        Ellipse(hdc, 275, 225, 284, 242); // 눈 (왼쪽)
        Ellipse(hdc, 525, 225, 534, 242); // 눈 (오른쪽)
        SelectObject(hdc, xxllipseBrush); // 흰색
        Ellipse(hdc, 278, 228, 281, 236); // 눈동자 (왼쪽)
        Ellipse(hdc, 528, 227, 531, 236); // 눈동자 (오른쪽)
        SelectObject(hdc, bBrush); // 검은색
        Ellipse(hdc, 383, 235, 417, 265); // 코
